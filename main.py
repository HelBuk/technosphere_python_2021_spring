    ...
    def validate_input(cls):
        ...
        if n == "Выход" or n == "выход":
            ...
            try:
                n = int(n)
                if 1 <= n <= 9:
                    if (cls.table[n - 1] == 'x') | (cls.table[n - 1] == 'o'):
                        print('Это место занято')
                        return cls.validate_input()
                    else:
                     return n
                else:
                    print('Необходимо ввести целое число от 1 до 9')
                    return cls.validate_input()
            except:
                print('Необходимо ввести целое число от 1 до 9')
                return cls.validate_input()
    ...
    def check_winner(cls):
        ...
        for each in res:
            if TicTacGame.table[each[0] - 1] == TicTacGame.table[each[1] - 1] == TicTacGame.table[each[2] - 1]:
                ...
                next = input('Чтобы начать новую игру введите "+": ')
                if next=="+":
                    print("Новая игра")
                    TicTacGame.table = list(range(1, 10))
                    return cls.start_game()
                else:
                    exit()
            elif TicTacGame.cnt == 9:
                print('Ничья')
                next = input('Введите "-" если хотите закончить игру, нажмите на любую клавишу, чтобы продолжить игру')
                if next == "-":
                    exit()
                else:
                    print("Новая игра")
